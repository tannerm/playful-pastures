...
class FLPostGridModule extends FLBuilderModule {
	...
	public function enqueue_scripts() {
		...
		if ( FLBuilderModel::is_builder_active() || ( in_array( $this->settings->layout, array(
					'grid',
					'columns'
				), true ) && $this->settings->show_comments_grid ) ) {
...
FLBuilder::register_module( 'FLPostGridModule', array(
	'layout'     => array(
		...
		'sections' => array(
			...
			'event_info' => [
				'title'  => __( 'Events', 'fl-builder' ),
				'fields' => [
					'event_template' => array(
						'title'         => __( 'Template', 'fl-builder' ),
						'type'          => 'editor',
						'label'         => '',
						'rows'          => 4,
						'media_buttons' => false,
						'wpautop'       => false,
						'default'       => '<em><strong>{start_date}, {start_time}</strong></em><br />{ministry} / {campus}<br />{venue:name} / {venue_address}',
			...
			'post_style'      => array(
				...
				'fields' => array(
					...
					'bg_color'   => array(
						...
						'preview'     => array(
							'type'     => 'css',
							'selector' => '.fl-post-feed-post, .fl-post-grid-post, .fl-callout',
