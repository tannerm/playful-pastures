	...
	<div class="fl-button-group-buttons" role="group" aria-label="<?php echo esc_attr( $settings->button_group_label ); ?>">
		...
		for ( $i = 0; $i < count( $settings->items ); $i++ ) :
			...
			$button_settings = array(
				...
				'icon_color'            => isset( $settings->items[ $i ]->icon_color ) ? $settings->items[ $i ]->icon_color : 'disable',
				'icon_size'             => isset( $settings->items[ $i ]->icon_size ) ? $settings->items[ $i ]->icon_size : 'disable',
				...
				'button_style'          => isset( $settings->items[ $i ]->button_style ) ? $settings->items[ $i ]->button_style : '',
				'button_size'           => isset( $settings->items[ $i ]->button_size ) ? $settings->items[ $i ]->button_size : '',
				'button_width'          => isset( $settings->items[ $i ]->button_width ) ? $settings->items[ $i ]->button_width : '',
				'button_color'          => isset( $settings->items[ $i ]->button_color ) ? $settings->items[ $i ]->button_color : '',
				'button_text'           => isset( $settings->items[ $i ]->button_text ) ? $settings->items[ $i ]->button_text : '',
			);
