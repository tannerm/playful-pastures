<?php
namespace Church\Integrations\EventsCalendar;

use Church\Integrations\EventsCalendar;
 ...
 * Class FilterMinisty
 */
class FilterMinistry extends \Tribe__Events__Filterbar__Filter {
	...
	protected function get_values() {
		...
		$source = get_terms( EventsCalendar::$_ministry_tax, array( 'orderby' => 'name', 'order' => 'ASC' ) );
	...
	protected function get_depth( $term, $level = 0 ) {
		...
		if ( 0 == $term->parent ) {
			...
			$level++;
			$term = get_term_by( 'id', $term->parent, EventsCalendar::$_ministry_tax );
	...
	protected function pre_get_posts( \WP_Query $query ) {
		...
		if ( 'select' === $this->type ) {
			...
			$categories = get_categories( array(
				'taxonomy' => EventsCalendar::$_ministry_tax,
